const fs = require('fs');

const html = fs.readFileSync('cimanow_watch.html', 'utf8');
const scriptMatch = html.match(/<script type="text\/javascript" language="Javascript">([\s\S]*?)<\/script>/);

if (!scriptMatch) {
    console.log("Script not found!");
    process.exit(1);
}

let jsCode = scriptMatch[1];
const polyfill = `
var document = {
    written: "",
    open: function() {},
    write: function(str) { this.written += str; },
    close: function() {}
};
`;

jsCode = polyfill + jsCode;
eval(jsCode);

fs.writeFileSync('scratch/cimanow_watch_decrypted_full.html', document.written, 'utf8');
console.log("Decoded HTML written to scratch/cimanow_watch_decrypted_full.html, size:", document.written.length);
