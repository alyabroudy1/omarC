...
| 28b | **The one authorised exception to rule 28** | 2026-08-03. `AwaitMainFrameUrl.probeJs` reads `#downloadbtn.href` on **rm.freex2line.online** and nowhere else, because the tokenised watch URL exists in that attribute and in no channel we can otherwise reach: `get-link.php` became a multipart POST (`WebResourceRequest` exposes no body, rule 32), the click is spent on an ad rather than a navigation (rule 36), and session identity, countdown timing, ad impressions and the popup-close cycle were each tested and eliminated. Scoped by `probeOnlyOnHosts`, which the engine checks *before* evaluating, so it cannot reach cimanow.cc — the `isBot()` stack check and the 47-byte decoy live in the decryptor's document, later in the flow, and remain untouched. It is a pure attribute read: no assignment, no synthetic click, nothing set on `window`. **Do not widen it.** Rule 28 still governs everything on cimanow's own pages. |
