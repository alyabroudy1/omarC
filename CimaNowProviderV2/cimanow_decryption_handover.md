...
| 32 | **Intercept a POST** | 2026-08-03, twice in one afternoon. `get-link.php` used to be a **GET with query parameters** (`?request_id=…&hmac_token=…&ch=…&fp=…` — see the 2026-07-02 HAR), and interception re-issued it faithfully. The site moved it to a `multipart/form-data` **POST**, and `WebResourceRequest` exposes no body — so the interceptor's `HttpURLConnection`, which is never told a method either, re-issued it as a **bodyless GET still carrying the multipart `Content-Type`**. Stripped of `request_id`/`hmac_token` it answered with the tokenless `https://cimanow.cc/pig/watching/`, cimanow served an ad interstitial for that (`<title>Redirect</title>`, 3,755 bytes, meta-refresh to an ad network), and the user watched a white screen for 300 s. Then *not* intercepting it removed the only place the URL was ever captured (`No watching URL captured`). The engine now refuses every non-GET, and the URL is taken from the navigation the user's tap produces — see rule 33. |
