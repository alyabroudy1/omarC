...
| 32 | **Intercept a POST** | 2026-08-03. `get-link.php` used to be a **GET with query parameters** (`?request_id=…&hmac_token=…&ch=…&fp=…` — 2026-07-02 HAR), and interception re-issued it faithfully. The site moved it to a `multipart/form-data` **POST**, and `WebResourceRequest` exposes no body — so the interceptor's `HttpURLConnection`, never told a method either, re-issued it as a **bodyless GET still carrying the multipart `Content-Type`**, a shape no browser produces. Then *not* intercepting it removed the only place the watch URL was ever captured (`No watching URL captured`). The engine now refuses every non-GET, and the URL comes from the navigation the user's tap produces — rule 33. |
| 33 | **Assume the timer page will navigate itself** | 2026-08-03. It fills in the watch button's `href` when the countdown ends and then **waits for a click** — there is no redirect to follow. So `NavigationStep.AwaitMainFrameUrl` holds the visible page open until the main frame lands on a tokenised cimanow `/watching/` URL, and the user presses the button exactly as they would in a browser. This needs neither the `get-link.php` response body nor the current token format, and reads nothing out of the page (rule 28). Two things it did break the first time: `overallTimeoutMs` must cover the button wait **plus** the surf, or the WebView is destroyed mid-surf; and a URL that merely *matches* is not enough — the tokenless fallback matches too, so `accept = ::isTokenisedWatchUrl` keeps it waiting. |
| 34 | **Gate the Chrome-TLS fallback on having cookies** | 2026-08-03. `executeDirectRequest`'s Tier-3 `ChromiumFetcher` retry required `cookiesForDomain.isNotEmpty()` — unreachable precisely when needed. cimanow refuses OkHttp's **TLS fingerprint**, not our cookies: the WebView pulls the full 269 KB page with **zero** cookies and `hasClearance=false`. So every request was "CF blocked", every block fell through to the CF-solve WebView, which every provider runs **FULLSCREEN** (`BaseProvider.skipHeadless = true`), which harvested no cookies because the site issues none — so nothing was cached and the next request did it again. One visible WebView per HTTP request, several per page load, each rendering unstyled because `CfBypassEngine` has no MIME-correcting interceptor and cimanow serves `.css`/`.js` as `text/html`. The condition is gone, CF-solved pages are now cached in `recentPages`, and the fallback verifies the body it got is not itself a block page. |
| 35 | **Mistake `https://cimanow.cc/pig/watching/` for a destination** | 2026-08-03, and this cost two wrong diagnoses before the log settled it. It is the **placeholder** in the watch button's `href`, sitting there until `get-link.php` replaces it with the real `…/<slug>/watching/?token=<64 hex>` (HAR, 335 KB document, Referer = blog-post page). Follow the placeholder and cimanow answers with a 3.7 KB `<title>Redirect</title>` stub that meta-refreshes to an ad — **every white page in this flow has been that stub**. It is not a tokenless fallback from a corrupted POST (the first theory) and not an ad waypoint en route to a player on freex2line (the second, which `NavigationEngine`'s own stale auto-approve comment suggests). Consequences of matching it: a lock pattern of `/(watch|watching)/` engages on the placeholder, pinning the session to a page that was never the page — and because `shouldOverrideUrlLoading` treats a **same-site** navigation away from a locked destination as terminal, the user's *next, correct* tap gets recorded as "the site sent us away" and kills the session. Both the destination lock and `AwaitMainFrameUrl` now require the query: `PLAYER_PAGE_PATTERN = cimanow\.[a-z]+/.+/watching/\?.+`. |
| 36 | **Expect the watch button's click to navigate** | 2026-08-03, verified in a real browser on-device (Brave). Tap 1 launched the **Temu app** via an ad deeplink; tap 2 opened a **YouTube ad in a new tab**; the original tab never moved. The site's design is: each click spends itself on an ad, the user closes the ad tab, and only after N rounds does the countdown page navigate for real. So a flow that waits for one click to produce a navigation waits forever. Related: the popup sink's retirement timer only ran when `loadPopupsInSink` was true, so with ads swallowed every window we handed the page stayed open for the whole session and its ad counter — which counts a *closed* window — never advanced. Sinks now always retire (`popupSinkTtlMs`, 1.5 s for CimaNow: past the 800 ms dwell, short enough to click again). |
| 37 | **Send `X-Requested-With` empty to "neutralise" it** | 2026-08-04, and it was our own long-standing bug. Measured on-device against cimanow.cc, same URL and UA: **no header → 200 `text/javascript`; empty value → 403 + 127 KB block page; single space → 200; `XMLHttpRequest` → 200; package name → 403.** `loadUrlInWebView` had been setting `""` for years, so every navigation carried the one value the site refuses — the watch page 403'd twice, and `jquery-3.5.1.min.js`, `owl.carousel.min.js`, `animate.css` and the anti-bot script all 403'd on re-issue, fell through to Chromium, and were refused for `text/html` MIME. Result: an unstyled page whose jQuery lazy-load never fired, so the video appeared only after a manual scroll. Now a single space — not `XMLHttpRequest`, because this header rides on **document** navigations and claiming a navigation is an XHR invites an AJAX-shaped reply instead of the page. The interceptor already omits it entirely (correct there: `HttpURLConnection` adds nothing of its own). Omitting it in `loadUrl` is **not** safe — WebView 150 fills it with the package name, which rule 32's fix proved. |
