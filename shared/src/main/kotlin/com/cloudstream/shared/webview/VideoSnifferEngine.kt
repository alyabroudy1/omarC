            ...
            webView.webViewClient = object : WebViewClient() {
                ...
                override fun shouldInterceptRequest(view: WebView?, request: WebResourceRequest?): android.webkit.WebResourceResponse? {
                    ...
                    if (requestUrl != null) {
                        ...
                        // The main document IS re-issued over IPv4, always.
                        //
                        // Tried and reverted 2026-07-29: leaving it to Chrome's stack keeps
                        // cf_clearance valid and removes the second Cloudflare challenge, but the
                        // token is then minted over IPv6 and the IPv4-only media edge answers 403 —
                        // verified in one run (`all/2001:16b8:…`, `Response code: 403`). A challenge
                        // costs seconds; an IPv6-pinned token is an unplayable link. Do not swap the
                        // trade back without solving the underlying conflict: cf_clearance is bound to
                        // the TLS fingerprint that solved it, and only OkHttp can pin the address
                        // family.
