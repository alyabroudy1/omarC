    ...
    private fun setupWebViewClient(
        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            override fun shouldInterceptRequest(
                ...
                // 1. Re-fetch cimanow CSS/JS to fix MIME types and drop X-Requested-With
                ...
                // 2. Intercept freex2line.online HTML documents to bypass Cloudflare block on X-Requested-With
                if (host.contains("freex2line.online") && accept.contains("text/html")) {
                    try {
                        val conn = java.net.URL(reqUrl).openConnection() as java.net.HttpURLConnection
                        ...
                        conn.connectTimeout = 15000
                        conn.readTimeout = 15000
                        ProviderLogger.d(TAG, "shouldInterceptRequest", "INTERCEPT HTML ${reqUrl.take(120)}")
                        if (conn.responseCode == 200) {
                            ...
                            ProviderLogger.w(TAG, "shouldInterceptRequest", "HTML Intercept non-200 (${conn.responseCode}) for ${reqUrl.take(80)}")
                            return null
                        }
                    } catch (e: Exception) {
                        ProviderLogger.w(TAG, "shouldInterceptRequest", "HTML Intercept failed: ${e.message}")
                    }
                }
...
Restored the original shortlink flow, because now that `freex2line` loads properly in the WebView, the standard countdown and click will work and properly unlock CimaNow.
