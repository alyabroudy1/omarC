...
class ConfigurableCloudflareKiller(
        ...
        when (val cookies = savedCookies[request.url.host]) {
            null -> {
                ...
                if(!(response.header("Server") in CLOUDFLARE_SERVERS && response.code in ERROR_CODES)) {
                    ...
                    response.close()
                    // Use internal resolve logic (headless)
                    resolveChallenge(request.url.toString())?.let {
     ...
     */
    suspend fun resolveChallenge(url: String, webView: WebView? = null): Response? {
        val originalHost = URI(url).host
        
        Log.i(CF_FLOW_TAG, "========================================")
        Log.i(CF_FLOW_TAG, ">>> [resolveChallenge] START")
        Log.i(CF_FLOW_TAG, ">>> URL: $url")
        Log.i(CF_FLOW_TAG, ">>> OriginalHost: $originalHost")
        Log.i(CF_FLOW_TAG, ">>> UA: ${userAgent?.take(50)}")
        Log.i(CF_FLOW_TAG, "========================================")

        // Check if we already have valid cookies (Double check)
        ...
        ConfigurableWebViewResolver(
            ...
            timeout = 120_000L, // Increased timeout 2min
            script = """
                (function() {
                    try {
        ...
        ).resolveUsingWebView(url, webView) { webViewRequest, isMainFrame ->
            ...
            // CRITICAL FIX: Only update on Main Frame navigation to avoid assets (like google-analytics) hijacking the "Final Domain"
            if (isMainFrame && reqHost != null && 
        ...
        Log.i(CF_FLOW_TAG, ">>> [POST_WEBVIEW] JS ExtractedCookies: ${extractedCookies.keys}")
        
        // Always try to fetch from CookieManager to get HttpOnly cookies (like cf_clearance) that JS cannot see
        val allCookies = extractedCookies.toMutableMap()
        
        // Helper to fetch and merge
        fun mergeFromUrl(targetUrl: String, context: String) {
            val cmCookies = getWebViewCookie(targetUrl)
            if (!cmCookies.isNullOrBlank()) {
                val parsed = parseCookieMap(cmCookies)
                Log.i(CF_FLOW_TAG, ">>> [COOKIE_MANAGER] Found ${parsed.size} cookies from $context ($targetUrl)")
                allCookies.putAll(parsed)
            }
        }
        
        mergeFromUrl("https://$finalDomain", "FinalDomain")
        if (finalDomain != originalHost) {
            mergeFromUrl("https://$originalHost", "OriginalHost")
        }
        // Try potential CF caching domains seen in redirects
        if (!finalDomain.contains("arabseed") && !finalDomain.contains("fasel")) {
             mergeFromUrl("https://$finalDomain", "ProxyDomain")
        }
        
        // Final check for cf_clearance specificity
        val hasClearance = allCookies.keys.any { it.equals("cf_clearance", ignoreCase = true) }
        Log.i(CF_FLOW_TAG, ">>> [MERGED] Total Cookies: ${allCookies.size} | Keys: ${allCookies.keys} | HasClearance: $hasClearance")

        // Store cookies for the FINAL domain
        if (allCookies.isNotEmpty()) {
            savedCookies[finalDomain] = allCookies
            if (finalDomain != originalHost) savedCookies[originalHost] = allCookies // Map original too just in case
            ...
            onCookiesExtracted?.invoke(finalDomain, allCookies)
            
            Log.i(CF_FLOW_TAG, "========================================")
            
            // Return success response if possible
            // Return success response if possible
            val cookieHeader = extractedCookies.entries.joinToString("; ") { "${it.key}=${it.value}" }
            return app.baseClient.newCall(
                 requestCreator("GET", url, headers = mapOf("Cookie" to cookieHeader))
            ).await() 
        }
    ...
    suspend fun resolveUsingWebView(
        ...
        fun destroyWebView() {
            if (isHeadless) {
                ...
                // Do not destroy external webview, just signal exit
                shouldExit = true
            }
        }
                ...
                override fun shouldInterceptRequest(
                    ...
                    if (interceptUrl.containsMatchIn(webViewUrl)) {
                        fixedRequest = webResourceRequestToRequest(request)?.also {
                            requestCallBack(it, request.isForMainFrame)
                        }
                    ...
                    if (additionalUrls.any { it.containsMatchIn(webViewUrl) }) {
                        webResourceRequestToRequest(request)?.also {
                            if (requestCallBack(it, request.isForMainFrame)) destroyWebView()
